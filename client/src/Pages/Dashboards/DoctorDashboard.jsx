...
const VerificationBanner = ({ status, onVerifyClick }) => {
  if (status === "verified") {
    return (
  ...
  const styles = {
    bg: isRejected ? "bg-red-50" : "bg-yellow-50",
    border: isRejected ? "border-red-400" : "border-yellow-400",
    icon: isRejected ? "text-red-400" : "text-yellow-400",
    text: isRejected ? "text-red-800" : "text-yellow-800",
    btnBg: isRejected
      ? "bg-red-400 hover:bg-red-500"
      : "bg-yellow-400 hover:bg-yellow-500",
    btnText: isRejected ? "text-red-900" : "text-yellow-900",
  };

  return (
    <div className="max-w-6xl mx-auto mb-6">
      <div
        className={`${styles.bg} border-l-4 ${styles.border} p-4 rounded-r-lg shadow-sm`}
      >
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button
            ...
            className={`w-full sm:w-auto flex-shrink-0 px-4 py-2 ${styles.btnBg} ${styles.btnText} rounded-lg text-sm font-semibold transition-colors shadow-sm`}
          >
            {isRejected ? "Re-submit Documents" : "Verify Account"}
          </button>
        </div>
      </div>
    </div>
...
const DoctorDashboard = () => {
  ...
  const sections = useMemo(
    () => doctorDashboardSections(navigate, () => setIsUploadModalOpen(true)),
    [navigate],
  );
