...
// Shape of a multer-s3 file as attached to req.files.
// Loosely typed for now — will tighten once middleware/multer-s3 typing is revisited.
...
export const addSymptom = asyncHandler(async (req: Request, res: Response) => {
  const { description, severity, onsetDate, notes, category } = req.body as {
    description?: string;
  ...
  const symptom = await Symptom.create({
    ...
    category,
    attachments:
      (req.files as UploadedFile[] | undefined)?.map((file) => ({
...
export const updateSymptom = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const { deletedAttachments, ...updates } = req.body as {
    ...
    if (deletedAttachments) {
      const deletedIds: string[] = JSON.parse(deletedAttachments);
      symptom.attachments = symptom.attachments.filter(
        (att: any) => !deletedIds.includes(att._id.toString()),
      );
    }
...
export const getSymptomById = asyncHandler(
  async (req: Request, res: Response) => {
...
export const deleteSymptom = asyncHandler(
  async (req: Request, res: Response) => {
...
export const analyzeSymptom = asyncHandler(
  async (req: Request, res: Response) => {
    const symptom = await Symptom.findOne({
      _id: req.params.id,
      patient: req.user.id,
...
`;
    ...
    try {
      let analysis: string = await askGemini(prompt);
